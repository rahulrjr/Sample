...
class TemplateBloc extends Bloc<TemplateEvent, TemplateState> {
  ...
  Stream<TemplateState> mapEventToState(
    ...
    if (event is GetTemplateInitialEvent) {
      yield TemplatesLoadingState();
    }
    if (event is GetTemplateEvent) {
      
      templateList = await loadFromAsset() ;
      yield TemplatesLoadedState(templateList);
    }
  }
}
...
List<TemplateModel> createTemplateModel(jsonString) {
  ...
  List<TemplateModel> list =
      jsonResult.map((val) => TemplateModel.fromJson(val)).toList();
